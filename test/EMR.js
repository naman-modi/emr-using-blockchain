...
contract('Initialize EMR Smart-Contract.', function (accounts) {
    ...
    it("Sign Up Patient.", function () {
        const _fullname = "Saurabh Kshirsaga";
        const _dob = "07 Sept 1999";
        const _sex = 0;
        const _marital = 1;
        const _postal_address = "PBN";
        const _city = "PBN";
        const _postal_code = "411402";
        const _contact_phone = "XXXXXXX";
        const _email = "Kshirsagarsrk@gmail.com";
        ...
        return EMR.deployed().then(function (instance) {
            ...
            return EMRInstance.PatientUpdate(_fullname + increment, _dob, _sex + increment,
                ...
                assert(error.message.indexOf('revert') >= 0, 'msg.value must equal number of tokens in wei SSS');
    ...
    it("Add Appointment.", function () {
        ...
        return EMR.deployed().then(function (instance) {
            ...
            //console.log(results);
            return EMRInstance.AppointmentUpdate(AppointmentIndex, 1, _remark,{ from: admin1 });
        }).then((receipt) => {
            assert.equal(receipt.logs.length, 1, 'triggers one event');
